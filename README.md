...
Всё обучение — в [notebooks/text_dl.ipynb](notebooks/text_dl.ipynb): четыре значимо разные
архитектуры на одних сплитах — TF-IDF с логрегом (baseline без сети), свёрточная сеть,
двунаправленный LSTM и дообучение трансформера `cointegrated/rubert-tiny2` через
высокоуровневый Trainer HuggingFace (подход не из лекций, под M3 — лёгкая tiny-модель и
сбалансированная подвыборка). Метрики по эпохам, кривые обучения, early stopping, подбор
гиперпараметров, macro-F1 и confusion matrix. Каждый запуск — отдельный run в DagsHub
(эксперимент `wb-review-sentiment`): гиперпараметры, per-epoch метрики, веса модели и
токенизатор. Аспекты недовольства (брак, размер, доставка, качество, цена) и темы негативных
отзывов через LDA — бизнес-вывод «почему низкий рейтинг» по 3 категориям. Сравнение
архитектур и выводы — [docs/text_dl.md](docs/text_dl.md)
...
Все метрики — на test, подробные сравнения — [docs/tabular_mlp.md](docs/tabular_mlp.md)
и [docs/text_dl.md](docs/text_dl.md)
